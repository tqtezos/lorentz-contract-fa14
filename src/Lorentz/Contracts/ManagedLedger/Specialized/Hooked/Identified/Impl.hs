...
module Lorentz.Contracts.ManagedLedger.Specialized.Hooked.Identified.Impl
  ( transfer
  , setHooks
  , getHooks
  , getBalance
...
transfer :: forall cp. NiceParameter cp => Entrypoint (WithWhichToken TransferParams) Storage
transfer = do
  runWithWhichToken @cp
  dip $ do
  ...
  swap
  dip swap
  withUserLedger (Just $ push 0) $ do
    swap
    dup
    mkTransferHookParams @cp transferHookIsTo
    dip $ do
      dup
      dip $ do
        coerce_ @TransferParams @(Address, (Address, Natural))
  ...
  dip $ do
    ...
    coerce_ @TransferParams @(Address, (Address, Natural))
    car
    withUserLedger Nothing $ do
      swap
      dup
      mkTransferHookParams @cp transferHookIsFrom
      dip $ do
        coerce_ @TransferParams @(Address, (Address, Natural))
        cdr
        cdr
...
getBalance :: forall cp. NiceParameter cp => Entrypoint (View (WithWhichToken GetBalanceParams) Natural) Storage
getBalance = view_ $ do
  unpair
  runWithWhichToken @cp
  fromNamed #owner; dip (toField #ledger); get
  ifNone (push 0) $ do
    coerce_ @LedgerValue @(Natural, TransferHooks)
    car

getTotalSupply :: forall cp. NiceParameter cp => Entrypoint (View ("whichToken" :! Address) Natural) Storage
getTotalSupply = do
  view_ $ do
    unpair
...
mint :: forall cp. NiceParameter cp => Address -> Entrypoint (WithWhichToken MintParams) Storage
...
burn :: forall cp. NiceParameter cp => Address -> Entrypoint (WithWhichToken BurnParams) Storage
